...
#Region "Public Functions"
    ...
    Protected Sub EnablerType(ByVal b As Boolean)
        Try
            ...
            divPgPanel.Visible = Not b
            divGvPanel.Visible = Not b
    ...
    Function GetItemsDT(ByVal Filter As String) As DataTable
        Try
            Dim dtItems As DataTable = DBContext.Getdatatable("select Category,SubCategory,Collection,Color,Size from tblItems where isnull(IsDeleted,0)=0  and " & Filter & "")
            Return dtItems
        Catch ex As Exception
            Return Nothing
        End Try
...
#Region "FillGrid Data Types"
    ...
    Sub FillDataTypes()
        Try
            Dim dtDataTypes As DataTable = DBContext.Getdatatable("Select * from tblLookup where isnull(IsDeleted,0)=0  and " + CollectConditions())
...
#Region "FillGrid Values"
    ...
    Private Sub FillValues()
        Try
            Dim dtValues As DataTable = DBContext.Getdatatable("Select * ,dbo.GetEnValue(RelatedValueId)  as ParentType from tblLookupValue where isnull(IsDeleted,0)=0  and LookupId='" + lblLookupId.Text + "'")
...
#Region "Add"
    ...
    Sub BindRelatedValues(ByVal Filter As String)
        Try
            Dim dtRType = DBContext.Getdatatable("Select RelatedTypeId,Type,Description from TblLookup where " + Filter + "  and Isnull(IsDeleted,0)=0 ")
...
#Region "Edit"
    ...
    Protected Sub Edit(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            Dim dt As DataTable = DBContext.Getdatatable("SELECT * From tblLookupValue where  Id ='" + lblLookupValueId.Text + "' ")
    ...
    Protected Sub Delete(ByVal Sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            If Not CheckLookupItem(LookupValueId) Then
                ...
                Return
            End If
            If DBContext.ExcuteQuery("Update tblLookupValue set IsDeleted=1,DeletedDate=getdate(),DeletedBy='" + UserId + "' where id= '" + LookupValueId + "'") = 1 Then
                FillValues()
    ...
    Function CheckValueNotExist(ByVal Operation As String) As Boolean
        ...
        Try
            Select Case Operation
                Case "Insert"
                    dtType = DBContext.Getdatatable("select Id from tblLookupValue where Value ='" + txtValue.Text + "' and LookupId ='" + lblLookupId.Text + "' and " + RTypeFilter + " and isnull(isDeleted,0)=0  ")
                Case "Update"
                    dtType = DBContext.Getdatatable("select Id from tblLookupValue where Value ='" + txtValue.Text + "' and LookupId ='" + lblLookupId.Text + "' and " + RTypeFilter + " and ID <>'" + lblLookupValueId.Text + "' and isnull(isDeleted,0)=0   ")
            End Select
    ...
    Function CheckCodeNotExist(ByVal Operation As String) As Boolean
        ...
        Try
            If txtCode.Text <> String.Empty Then
                Select Case Operation
                    Case "Insert"
                        dtType = DBContext.Getdatatable("select Id from tblLookupValue where Code ='" + txtCode.Text + "' and LookupId ='" + lblLookupId.Text + "' and isnull(isDeleted,0)=0  ")
                    Case "Update"
                        dtType = DBContext.Getdatatable("select Id from tblLookupValue where Code ='" + txtCode.Text + "' and LookupId ='" + lblLookupId.Text + "' and ID <>'" + lblLookupValueId.Text + "' and isnull(isDeleted,0)=0 ")
                End Select
    ...
    Protected Sub PhotoUploaded(ByVal sender As Object, ByVal e As EventArgs)
        Try
            ...
            If fuPhoto.PostedFile IsNot Nothing Then
                ...
                Dim filePath As String = "~/Settings_Photos/" & fuPhoto.FileName
