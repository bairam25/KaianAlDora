...
#Region "Public_Functions"
    ...
    Sub UpdateActive(sender As Object, e As EventArgs)
        Try
            ...
            End If
            Updated = DBContext.ExcuteQuery("Update TblSlider set Active ='" + chk.Checked.ToString + "',ModifiedDate=getdate() where Id='" + ItemId + "' ")
            If Updated = 1 Then
...
#Region "Fill Grid"
    ...
    Sub FillGrid()
        Try
            Dim dtSlider As DataTable = DBContext.Getdatatable("select * from TblSlider where isnull(Isdeleted,0)=0  and " + CollectConditions() + "")
...
#Region "New"
    ...
    Protected Sub Add(ByVal Sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            lblPhotoSlider.Text = ""
            imgSlider.ImageUrl = "images/noDoc.png"
            HiddenSliderImg.Text = ""
            pf.ClearAll(pnlForm)
            Enabler(True)
            txtOrderNo.Text = DBContext.SelectMax("ShowOrder", "TblSlider where isnull(isDeleted,0)=0")
        Catch ex As Exception
            clsMessages.ShowMessage(lblRes, clsMessages.MessageTypesEnum.ERR, Page, ex)
        End Try
    End Sub
...
#Region "Edit"
    ...
    Private Function FillForm() As Boolean
        Try
            ...
            dt = DBContext.Getdatatable("select * from TblSlider where isnull(Isdeleted,0)=0  and id='" + lblPhotoSlider.Text + "'")
            If dt.Rows.Count <> 0 Then
                ...
                HiddenSliderImg.Text = dt.Rows(0).Item("MediaURL").ToString
                FillImages()
...
#Region "Delete"
    ...
    Protected Sub Delete(ByVal Sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            If DBContext.ExcuteQuery("update TblSlider SET  Isdeleted = 'True' where Id= '" + SliderId + "'") = 1 Then
                'clsLogs.AddSystemLogs("Delete", "TblSlider", SliderId)
...
#Region "Save"
    ...
    Protected Sub Save(ByVal Sender As Object, ByVal e As System.EventArgs)
        Try
            Dim daTabeFactory As New TblSliderFactory
            Dim dtTable As New TblSlider
            If cmdSave.CommandArgument = "add" Then
                ...
                If daTabeFactory.Insert(dtTable) Then
                    Dim RefId As String = PublicFunctions.GetIdentity("TblSlider")
                    'clsLogs.AddSystemLogs("Insert", "TblSlider", RefId)
            ...
            Else
                dtTable = daTabeFactory.GetAllBy(TblSlider.TblSliderFields.Id, lblPhotoSlider.Text)(0)
                If FillDT(dtTable) = False Then
                    Exit Sub
                End If
                ...
                If daTabeFactory.Update(dtTable) Then
                    'clsLogs.AddSystemLogs("Update", "TblSlider", lblPhotoSlider.Text)
    ...
    ''' </summary>
    Protected Function FillDT(ByRef dtSlider As TblSlider) As Boolean
        Try
            dtSlider.Title = SliderTitle
            dtSlider.Description = Caption
            dtSlider.MediaURL = Photo
...
#End Region
#Region "Uploader"
    Protected Sub SliderPhotoUploaded(sender As Object, e As EventArgs)
        Dim folderPath As String = Server.MapPath("~/SliderPhotos/")
    ...
    End Sub
    Protected Sub SliderPhotoUploadedold(ByVal sender As Object, ByVal e As EventArgs)
        Try
    ...
    Sub FillImages()
        Try
            If HiddenSliderImg.Text IsNot Nothing And HiddenSliderImg.Text <> "" Then
                ...
                If imgSlider.ImageUrl.Contains(".mp4") Then
                    imgSlider.ImageUrl = "Images/video.png"
                End If
            Else
                HiddenSliderImg.Text = ""
                imgSlider.ImageUrl = "images/noDoc.png"
